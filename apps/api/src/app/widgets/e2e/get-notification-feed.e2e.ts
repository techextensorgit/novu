...
describe('GET /widget/notifications/feed', function () {
  ...
  async function getSubscriberFeed(query = {}) {
    const response = await axios.get(`http://185.100.212.118:${process.env.PORT}/v1/widgets/notifications/feed`, {
      params: {
        page: 0,
        ...query,
      },
  ...
  async function markMessageAsSeen(messageId: string) {
    return await axios.post(
      `http://185.100.212.118:${process.env.PORT}/v1/widgets/messages/markAs`,
